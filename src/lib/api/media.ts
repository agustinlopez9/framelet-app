...
import type { MediaItem } from '@/features/portfolio/types';
...
export async function listMedia(portfolioId: string): Promise<MediaItem[]> {
  const [images, videos] = await Promise.all([listImages(portfolioId), listVideos(portfolioId)]);
...
export async function reorderMedia(items: MediaItem[], portfolioId: string): Promise<void> {
  const imageItems = items.filter(
    (i): i is MediaItem & { mediaType: 'image' } => i.mediaType === 'image',
  );
  const videoItems = items.filter(
    (i): i is MediaItem & { mediaType: 'video' } => i.mediaType === 'video',
  );

  const image_ids = imageItems.map((i) => i.id);
  const video_ids = videoItems.map((v) => v.id);
  const image_positions = imageItems.map((i) =>
    items.findIndex((item) => item.id === i.id && item.mediaType === 'image'),
  );
  const video_positions = videoItems.map((v) =>
    items.findIndex((item) => item.id === v.id && item.mediaType === 'video'),
  );
